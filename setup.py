...
class BaseBuildCommand(Command):
    ...
    def _run_npm_command(self, cmd, env=None):
        self._run_command(["yarn"] + cmd, env=env)
...
class BuildAssetsCommand(BaseBuildCommand):
    ...
    def initialize_options(self):
        self.work_path = os.path.join(ROOT_PATH, "self_blog/static/self_blog")
        self.asset_json_path = os.path.join(self.work_path, "dist", "assets.json")
    ...
    def _get_package_version(self):
        """
        Attempt to get the most correct current version of SelfBlog.
        """
        pkg_path = ROOT_PATH
        ...
        try:
            ...
            log.info(f"pulled version information from 'self_blog' module. {self_blog.__file__}")
            version = self.distribution.get_version()
            build = self_blog.__build__
        finally:
            sys.path.pop(0)
    ...
    def _build(self):
        ...
        if not version_info["version"] or not version_info["build"]:
            log.fatal("Could not determine blog version or build")
            sys.exit(1)
        ...
        try:
            ...
            traceback.print_exc()
            log.fatal("unable to build Blog's static assets!")
            sys.exit(1)
    ...
    def _build_static(self):
        ...
        env = dict(os.environ)
        env["BLOG_STATIC_DIST_PATH"] = self.dispatch_static_dist_path
    ...
    def dispatch_static_dist_path(self):
        return os.path.abspath(os.path.join(self.build_lib, "self_blog/static/self_blog/dist"))
...
VERSION = "0.1.0.dev0"
IS_LIGHT_BUILD = os.environ.get("BLOG_LIGHT_BUILD") == "1"
...
class SelfBlogSDistCommand(SDistCommand):
...
class SelfBlogBuildCommand(BuildCommand):
    def run(self):
        if not IS_LIGHT_BUILD:
...
class SelfBlogDevelopCommand(DevelopCommand):
    def run(self):
...
cmdclass = {
    "sdist": SelfBlogSDistCommand,
    "develop": SelfBlogDevelopCommand,
    "build": SelfBlogBuildCommand,
...
setup(
    name="PhuocBao_self_blog",
    version=VERSION,
    long_description=long_description,
    long_description_content_type="text/markdown",
    author="Huynh Phuoc Bao",
    ...
    package_dir={"": "self_blog"},
    packages=find_packages("self_blog"),
    python_requires=">=3.7",
    ...
    entry_points={
        "console_scripts": ["blog = self_blog.cli:entrypoint"],
